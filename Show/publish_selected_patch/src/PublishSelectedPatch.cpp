...
#include "publish_selected_patch/PublishSelectedPatch.h"
...
void PublishSelectedPatch::updateTopic()
{
    ...
    pc_subs_ =  nh_.subscribe(subs_cloud_topic_.c_str(),1,&PublishSelectedPatch::PointCloudsCallback, this);

    ROS_INFO_STREAM_NAMED("PublishSelectedPatch.updateTopic", "Publishing rviz selected points on topic " <<  nh_.resolveName (rviz_cloud_topic_) );//<< " with frame_id " << context_->getFixedFrame().toStdString() );
    ROS_INFO_STREAM_NAMED("PublishSelectedPatch.updateTopic", "Publishing real selected points on topic " <<  nh_.resolveName (real_cloud_topic_) );//<< " with frame_id " << context_->getFixedFrame().toStdString() );
    ROS_INFO_STREAM_NAMED("PublishSelectedPatch.updateTopic", "Publishing bounding box marker on topic " <<  nh_.resolveName (bb_marker_topic_) );//<< " with frame_id " << context_->getFixedFrame().toStdString() );
...
void PublishSelectedPatch::PointCloudsCallback(const sensor_msgs::PointCloud2ConstPtr &pc_msg)
{
    ...
    {
        ROS_INFO_STREAM_NAMED( "PublishSelectedPatch::PointCloudsCallback", "Received PC");
    }
...
int PublishSelectedPatch::processKeyEvent( QKeyEvent* event, rviz::RenderPanel* panel )
{
        if(event->type() == QKeyEvent::KeyPress)
        {
            ...
            {
                ROS_INFO_STREAM_NAMED( "PublishSelectedPatch::processKeyEvent", "Cleaning ALL previous selection (selected area and points).");
            ...
            {
                ROS_INFO_STREAM_NAMED( "PublishSelectedPatch.processKeyEvent", "Reusing the LAST selected area to find a NEW bounding box.");
            ...
            {
                ROS_INFO_STREAM_NAMED( "PublishSelectedPatch.processKeyEvent",
                ...
                ROS_INFO_STREAM_NAMED("PublishSelectedPatch._processSelectedAreaAndFindPoints",
...
int PublishSelectedPatch::processMouseEvent( rviz::ViewportMouseEvent& event )
{
    ...
    if( selecting_ )
    {
        if( event.leftUp() )
        {
            ROS_INFO_STREAM_NAMED( "PublishSelectedPatch.processKeyEvent", "Using selected area to find a new bounding box and publish the points inside of it");
            this->_processSelectedAreaAndFindPoints();
        }
    }
    return flags;
...
int PublishSelectedPatch::_processSelectedAreaAndFindPoints()
{
    rviz::SelectionManager* sel_manager = context_->getSelectionManager();
    rviz::M_Picked selection = sel_manager->getSelection();
    rviz::PropertyTreeModel *model = sel_manager->getPropertyModel();
    int num_points = model->rowCount();
    ROS_INFO_STREAM_NAMED( "PublishSelectedPatch._processSelectedAreaAndFindPoints", "Number of points in the selected area: " << num_points);
...
int PublishSelectedPatch::_publishAccumulatedPoints()
{
    ROS_INFO_STREAM_NAMED("PublishSelectedPatch._processSelectedAreaAndFindPoints",
    ...
    ROS_WARN_STREAM_NAMED("PublishSelectedPatch._processSelectedAreaAndFindPoints",
...
PLUGINLIB_EXPORT_CLASS( rviz_plugin_selected_points_publisher::PublishSelectedPatch, rviz::Tool )
